...
├── data/                   ← benchmark outputs from earlier analysis runs
...
These were derived from `data/20260317_Data_Greenings_clean_NG.xlsx` using
`old/convert_to_csv.py` and manually enriched in `modeled_input_data/`.
...
|---|---|---|
| `old/convert_to_csv.py` | Converted Excel to three sector CSVs | Input files already in `data/` |
...
Output CSVs generated by the old scripts are preserved in `data/` and `old/*/output/`.
